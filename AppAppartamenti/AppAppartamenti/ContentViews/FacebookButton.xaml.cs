...
namespace AppAppartamenti.ContentViews
{
    public partial class FacebookButton : ContentView
    {
        ...
        private async void btnAccediFacebook_Clicked(object sender, EventArgs e)
        {
            try
            {
                bool answer = await App.Current.MainPage.DisplayAlert("Promuovocasa.it vuole utilizzare Facebook.com per accedere", "Questo permette all'app e al sito web di accedere alle tue informazioni.", "Si", "No");
                if (answer)
                {
